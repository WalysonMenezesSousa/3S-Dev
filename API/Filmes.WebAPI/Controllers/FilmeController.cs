...
public class FilmeController : ControllerBase
{
    ...
    public IActionResult GetById(Guid id)
    {
        try
        {
            return Ok(_filmeRepository.BuscarPorId(id));//Busca o filme pelo id e retorna o resultado
        }
        catch (Exception erro)
        {
            return BadRequest(erro.Message);//Retorna um status code 400 (Bad Request) com a mensagem de erro
        }
    }
    ...
    public async Task<IActionResult> Post([FromForm] FilmeDTO filme)
    {
        if (String.IsNullOrWhiteSpace(filme.Nome))
            return BadRequest("O nome e o Gênero do filme é obrigatório.");
